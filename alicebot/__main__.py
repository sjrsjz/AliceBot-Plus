...
onebot_package = moduleloader.ModuleLoader(
    str(pathlib.Path(__file__).parent / "onebot"), log_func=log_func
)
onebot_api_module = onebot_package.load_module(
    "api", hot_reload=True, log_func=log_func
)

bot_package = moduleloader.ModuleLoader(
    str(pathlib.Path(__file__).parent / "entity"), log_func=log_func
)
...
try:
    protocol_config.load(
        str(pathlib.Path(__file__).parent / "config" / "protocol.json")
    )
    ...
    log_func(
        "INFO",
        "Config",
        "Default protocol config created, please configure the protocol and restart the program.",
    )
    sys.exit(0)
    ...
    while not close_event.is_set():
        ...
        try:
            ...
            if "status" in message and "echo" in message:
                ...
                continue
            if (
                message["post_type"] == "meta_event"
                and message["meta_event_type"] == "heartbeat"
            ):
                        ...
                        for idx in range(len(status.bot_tasks)):
                            if status.bot_tasks[idx]["task"] == task:
                                log_func(
                                    ...
                                    "Time:",
                                    time.time() - status.bot_tasks[idx]["start_time"],
                                )
            ...
            if timeout_count > 10:
                log_func(
                    "ERROR",
                    "Websocket",
                    "Cannot hold connection! is the bot backend still alive? Reconnecting...",
                )
                break
            continue
        except Exception as e:
...
def init(status: Status):
    ...
    log_file = (
        ...
        / "logs"
        / f"{time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime())}.log"
    )
    log_file.parent.mkdir(parents=True, exist_ok=True)

    tui = TUI.RichTUI(
        update_status, close_server, open(log_file, "a", encoding="utf-8")
    )
    ...
    update_machine_status_thread = threading.Thread(
        target=update_machine_status, daemon=True
    )
...
async def server(ws_url=protocol_config.ws_url):
    ...
    while not close_event.is_set():
        try:
            # 添加token鉴权支持(OneBotv11协议)
            extra_headers = {}
            if protocol_config.token:
                extra_headers["Authorization"] = f"Bearer {protocol_config.token}"
            ...
            if "self_id" in response:
                ...
                log_func(
                    "ERROR", "Websocket", "Failed to connect to bot backend: ", response
                )
                await asyncio.sleep(5)
                continue
        except websockets.exceptions.ConnectionClosedError:
            log_func(
                "ERROR", "Websocket", "Connection closed, reconnect after 5 seconds"
            )
            if ws:
                await ws.close()
            await asyncio.sleep(5)
            ...
            log_func(
                "ERROR", "Websocket", f"Error: {str(e)}, reconnect after 5 seconds"
            )
            if ws:
                await ws.close()
            await asyncio.sleep(5)
