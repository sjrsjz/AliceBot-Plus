...
async def setup_browser():
    try:
        ...
        if sys.platform.startswith("linux"):
            browser = await launch(headless=True,executablePath="/usr/bin/chromium-browser",dumpio=True, args=['--no-sandbox', '--disable-setuid-sandbox'],userDataDir=cache_dir)
            return browser
        else:
...
async def get_webpage(browser, url, only_text=False, max_token=2048):
    try:
        ...
        text = ""
        for i in soup.find_all(["p", "span", "h1", "h2", "h3", "h4", "h5", "h6", "code", "img", "cite" , "a", "ul", "ol", "li" , "br", "dl", "dt", "dd"]):
...
def MarkdownRenderer(browser):
    ...
    class _MarkdownRenderer:
        ...
        class GenerateImgTag(SimpleTagPattern):
            ...
            def handleMatch(self, m):
                ...
                if not_middle:
                    ...
                    return ET.Element("img", alt=alt, src=f"data:image/{img_format};base64,{img}", style="display: block; margin: 0 auto;")
        ...
        class WolframAlphaExtension(Extension):
            def extendMarkdown(self, md):
                md.parser.blockprocessors.register(_MarkdownRenderer.WolframAlphaProcessor(md.parser), 'wolframalpha', 184)
    ...
    def convert_markdown_to_html(text):
        try:
            ...
            md = markdown.Markdown(extensions=[
                ...
                _MarkdownRenderer.DelExtension(),_MarkdownRenderer.InsExtension(),_MarkdownRenderer.SubExtension(),_MarkdownRenderer.SupExtension(),
                _MarkdownRenderer.MarkExtension(),_MarkdownRenderer.UnderlineExtension(),_MarkdownRenderer.SmallExtension(),_MarkdownRenderer.TtExtension(),
                "markdown_checklist.extension",_MarkdownRenderer.ImgExtension(),_MarkdownRenderer.TableExtension(),
                _MarkdownRenderer.WolframAlphaExtension(),_MarkdownRenderer.PieChartExtension(),_MarkdownRenderer.FontExtension()
                ])
